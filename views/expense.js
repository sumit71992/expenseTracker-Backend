  ...
  try {
    ...
    if (!desc.title) {
      const expense = await axios.post(
        "http://3.109.42.131:3000/expense/addexpense",
        obj,
        {
  ...
  try {
    if (e.target.classList.contains("del")) {
      const deletedExpense = await axios.delete(
        "http://3.109.42.131:3000/expense/" + e.target.id,
        {
          headers: { Authorization: token },
        }
      );
...
premium.addEventListener("click", async (e) => {
  if (e.target.classList.contains("buy_premium")) {
    const response = await axios.get("http://3.109.42.131:3000/order/buypremium", {
      headers: { Authorization: token },
    });
    ...
    var options = {
      ...
      handler: async function (response) {
        await axios.post(
          "http://3.109.42.131:3000/order/updatestatus",
          {
...
leaderboard.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("leaderboard")) {
    axios
      .get("http://3.109.42.131:3000/expense/leaderboard", {
        headers: { Authorization: token },
      })
      .then((res) => {
        console.log(res);
